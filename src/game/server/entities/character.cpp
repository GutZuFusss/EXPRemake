...
void CCharacter::HandleWeaponSwitch()
{
	...
	else if(WantedWeapon == WEAPON_LASER && m_aWeapons[WEAPON_FREEZER].m_Got)
...
{
	if(m_Health >= GetPlayer()->MaxHealth())
		return false;
	m_Health = clamp(m_Health+Amount, 0, GetPlayer()->MaxHealth());
	return true;
}

bool CCharacter::IncreaseArmor(int Amount)
{
	if(m_Armor >= GetPlayer()->MaxHealth())
		return false;
	m_Armor = clamp(m_Armor+Amount, 0, GetPlayer()->MaxHealth());
	return true;
}
...
void CCharacter::Snap(int SnappingClient)
{
	...
	if(m_ActiveWeapon == WEAPON_KAMIKAZE)
		...
		pCharacter->m_Weapon = WEAPON_LASER;
	else

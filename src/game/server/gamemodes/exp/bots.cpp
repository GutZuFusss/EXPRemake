		...
		if(m_Boss.m_Spawn.m_Spawned)
		{
			...
			else
			{
				if((float)Server()->Tick() > m_Boss.m_ShieldTimer)
				{
					...
					for(int s = 0; s < 3; s++)
					{
						m_Boss.m_apShieldIcons[s] = new CPickup(&GameServer()->m_World, 1, GameServer()->m_apPlayers[m_Boss.m_ClientID]->GetCharacter()->GetPos());
...
void CGameControllerEXP::BotSpawn(CBotSpawn *pSpawn)
{
	...
	GameServer()->OnClientConnected(BID, false, true);
...
void CGameControllerEXP::RemoveBot(int ID, bool Killed)
{
	...
	if(Killed)
	{
		CPickup *pPickup = new CPickup(&GameServer()->m_World, 0, GameServer()->m_apPlayers[ID]->m_LastPos);
		pPickup->CreateRandomFromBot(GameServer()->m_apPlayers[ID]->m_BotLevel);
	}
